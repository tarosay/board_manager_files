...
- **追加のみで既存 API は変更なし** — ヘッダオンリーの inline メソッド 1 つなので、使わなければコードは生成されない。`NeoPixelmin_ring` は Flash 3,532 バイト / RAM 212 バイトのまま変化なし。シグネチャは Adafruit_NeoPixel と同じ `uint8_t *getPixels(void) const`
- **使うときの約束** — バイト並びは色順に従う（`NEO_GRB` なら G, R, B の順）、入っているのは輝度スケール前の素の値、境界チェックは無いので `numPixels() * 3` バイトに収めるのは呼び出し側の責任
- **スケッチ例 `NeoPixelmin_getPixels` を追加** — `setPixelColor` と `getPixels` を左右に並べて結果が一致することを目で確かめられる。実機の WS2812B 12 連リングで動作確認済み
